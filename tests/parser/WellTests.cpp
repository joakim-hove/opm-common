...
namespace {
    namespace WCONHIST {
        ...
        Opm::WellProductionProperties properties(const std::string& input) {
            ...
            Opm::WellProductionProperties hist("H");
...
BOOST_AUTO_TEST_CASE(CMODE_DEFAULT) {
    const Opm::WellProductionProperties Pproperties("W1");
    const Opm::WellInjectionProperties Iproperties("W2");
...
BOOST_AUTO_TEST_CASE(WELL_CONTROLS) {
    ...
    BOOST_CHECK_THROW(prod.controls(st, 0), std::out_of_range);
